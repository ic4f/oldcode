...
Namespace webpages
    ...
    Public Class dstoryList
        ...
        Private Sub BindData()
            ...
            If (ddlFeatured.SelectedIndex > 0 Or ddlStatus.SelectedIndex > 0 _
              Or ddlLabel.SelectedIndex > 0 Or ddlTag.SelectedIndex > 0 Or keywords <> "") Then
                pages = New dt.DStoryPageData().GetDStoryPagesWithQuery(makeQuery(keywords))
            Else
                pages = New dt.DStoryPageData().GetDStoryPages(dgrPages.SortExpression)
            End If
        ...
        Private Function makeQuery(ByVal keywords As String) As String
            ...
            If (ddlFeatured.SelectedValue = "1") Then
                sb.Append(" and d.isfeatured = 1 ")
            ElseIf (ddlFeatured.SelectedValue = "2") Then
                sb.Append(" and d.isfeatured = 0 ")
            End If
            ...
            If (keywords <> "") Then
                ...
                If arr.Length > 0 Then
                    ...
                    For Each term As String In arr
                        sbKeywords.AppendFormat(" (p.pagetitle like '%{0}%' or p.contenttitle like '%{0}%' or p.body like '%{0}%' or p.bodydraft like '%{0}%' or p.admincomment like '%{0}%' or d.donordisplayedname like '%{0}%' or d.summary like '%{0}%') and ", term)
                    Next
