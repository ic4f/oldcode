...
Namespace contentlibrary
    ...
    Public Class imageInfo
        ...
        Private Sub BindData()
            btnDelete.Attributes("onclick") = "javascript:return confirm('Are you sure you want to delete this image?')"
        ...
        Private Sub BindInfo(ByVal myfile As dt.File)
            ...
            Dim url As String = ba.UrlHelper.GetImageUrl(myfile.Id, myfile.Extension)
            Dim urlt As String = ba.UrlHelper.GetImageThumbnailUrl(myfile.Id, myfile.Extension)
            ltrUrl.Text = "<a href='" + url + "' target='_blank'>" + url + "</a>"
            ltrId.Text = myfile.Id.ToString
            ltrImage.Text = "<a href='" + url + "' target='_blank'><img src='" + urlt + "' border='1'></a>"
        End Sub
        ...
        Private Function HasDependencies(ByVal myfile As dt.File, ByVal fd As dt.FileData) As Boolean
            ...
            If countPages > 0 Then
                If countPages = 1 Then
                    ltrInboundPageLinks.Text = countPages.ToString + " page refers to this image"
                Else
                    ltrInboundPageLinks.Text = countPages.ToString + " pages refer to this image"
                End If
        ...
        Private Sub btnEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEdit.Click
            Response.Redirect("imageEdit.aspx?Id=" + GetId().ToString)
        End Sub

        Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
            Dim fileId As Integer = GetId()
            Dim f As New dt.File(fileId)
            File.Delete(Server.MapPath(ba.UrlHelper.GetImageThumbnailUrl(fileId, f.Extension)))
            File.Delete(Server.MapPath(ba.UrlHelper.GetImageUrl(fileId, f.Extension)))
            Dim fd As New dt.FileData
            fd.Delete(fileId)
            Response.Redirect("imageList.aspx")
        End Sub
