...
namespace Giggle.Indexing
{
    ...
    public class DataLoader
    {
        ...
        public float[] PageRanks { get { return pageRanks; } }

        public short[] DocLinkCounts { get { return linkLoader.DocLinkCounts; } }

        public short[][] DocCitations { get { return linkLoader.DocCitations; } }

        public short[][] DocLinks { get { return linkLoader.DocLinks; } }

        public short[] DocCitationCounts { get { return linkLoader.DocCitationCounts; } }
        ...
        private void load(string sourceDirectory, float c)
        {
            ...
            loadLinks(sourceDirectory);
            loadPageRanks(sourceDirectory, c);
        }
        ...
        private void loadPageRanks(string sourceDirectory, float c)
        {
            PageRankLoader prl = new PageRankLoader(NumberOfDocs);
            string prFile = sourceDirectory + d.Helper.SOURCE_PAGERANK_FILE;
            if (File.Exists(prFile))
                prl.LoadPageRanks(prFile);
            else
                prl.CalculateAndWritePageRanks(linkLoader.DocLinks, linkLoader.DocCitations, c, sourceDirectory);

            pageRanks = prl.PageRanks;
        }

        private void loadLinks(string sourceDirectory)
        {
            string linksFile = sourceDirectory + d.Helper.SOURCE_DOCLINKS_FILE;
            string citationsFile = sourceDirectory + d.Helper.SOURCE_DOCCITATIONS_FILE;
            linkLoader = new LinkLoader(linksFile, citationsFile, NumberOfDocs);
        }
