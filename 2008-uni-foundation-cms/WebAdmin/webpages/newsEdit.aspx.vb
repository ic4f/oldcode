...
Namespace webpages
    ...
    Public Class newsEdit
        ...
        Protected cbxHomepage As CheckBox
        Protected cbxHighlight As CheckBox
        ...
        Private Sub BindData()
            ...
            Dim mynews As New dt.NewsPage(New dt.NewsPageData().GetIdByPageId(pageId))
            ...
            tbxSummary.Text = mynews.Summary
            cbxHomepage.Checked = mynews.IsOnHomepage
            cbxHighlight.Checked = mynews.IsHighlighted

            If mynews.HasImage Then
                ltrImage.Text = "<br><br><img src='" + ba.UrlHelper.GetNewsImageUrl(mynews.Id) + "' border='1'>"
            End If
        ...
        Protected Overrides Function SaveItem() As Integer
            If Page.IsValid Then
                ...
                If result > 0 Then
                    Dim mynews As New dt.NewsPage(New dt.NewsPageData().GetIdByPageId(pageId))
                    mynews.IsOnHomepage = cbxHomepage.Checked
                    mynews.IsHighlighted = cbxHighlight.Checked
                    mynews.Summary = tbxSummary.Text.Trim

                    Dim path As String = Server.MapPath(ba.UrlHelper.GetNewsImageUrl(mynews.Id))
                    If tbxFile.PostedFile.ContentLength > 0 Then
                        mynews.HasImage = True
                        ProcessImage(mynews.Id)
                    ElseIf cbxRemove.Checked Then
                        mynews.HasImage = False
                        File.Delete(path)
                    End If
        ...
        Private Sub ProcessImage(ByVal newsId As Integer)
            ...
            ih.MakeImage(
             tbxFile.PostedFile.InputStream, Server.MapPath(ba.UrlHelper.GetNewsImageUrl(newsId)),
             ba.ConfigurationHelper.NewsImageWidth,
             ba.ConfigurationHelper.NewsImageHeight)
        End Sub

        Protected Overrides Sub PublishItem()
            'this ensures that the displayed "published date" of a news page is changed only once
            Dim mypageId As Integer = GetId()
            Dim mypage As New dt.Page(mypageId)
            If Not mypage.IsPublished Then
                Dim dn As New dt.NewsPageData
                dn.Publish(dn.GetIdByPageId(mypageId))
            End If
            MyBase.PublishItem()
        End Sub
