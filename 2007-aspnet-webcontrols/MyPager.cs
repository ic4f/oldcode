...
namespace WebControls
{
    public class MyPager : System.Web.UI.WebControls.WebControl
    {
        ...
        private void SetPagerNumbers(int totalPages, int totalRecords)
        {
            ...
            lblLastRecord.Text = Math.Min(firstRecord + PageSize, totalRecords).ToString();

            lblFirstRecord.Text = firstRecord.ToString();

            lblTotalPages.Text = totalPages.ToString();
        }

        private void SetPagerList1(int totalPages)
        {
        ...
        protected override void CreateChildControls()
        {
            ...
            Controls.Add(lblFirstRecord);
            Controls.Add(lblLastRecord);
        }
        ...
        {
            double totalRecords = Convert.ToDouble(lblTotalRecords.Text);
            int totalPages = (int)Math.Ceiling((double)totalRecords / PageSize);
            ddlPages.Items.Clear();
            for (int i = 1; i <= totalPages; i++)
                ddlPages.Items.Add(new ListItem(i.ToString(), i.ToString()));
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            lblFirstRecord.RenderControl(writer);
            writer.Write("-");
            lblLastRecord.RenderControl(writer);
            writer.Write("</b> of <b>");
            lblTotalRecords.RenderControl(writer);
            writer.Write("</b></td>");
